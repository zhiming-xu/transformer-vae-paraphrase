...
class TCVAE():
        ...
        with tf.variable_scope("encoder") as scope:
            ...
            for i in range(self.num_layers):
                with tf.variable_scope("num_layers_{}".format(i)):
                    ...
                    outputs = feedforward(inputs, [self.num_units * 2, self.num_units], \
                              is_training=self.is_training, dropout_rate=self.dropout_rate, scope="f1")
                    # add and layer_norm
                    outputs = outputs + inputs
                    inputs = normalize(outputs)
    ...
    def infer_step(self, sess, data, no_random=False, batch_idx=0, which=0):
        input_ids, input_scopes, input_positions, input_masks, input_lens, input_which, \
        targets, weights, input_windows = self.get_batch(data, no_random, batch_idx, which=which)
        start_pos = []
        given = []
        ans = []
        predict = []
        for i in range(self.hparams.batch_size):
            ...
            given.append(input_ids[i][:start_pos[i]] + [UNK_ID] * self.max_single_length + \
                         input_ids[i][start_pos[i] + self.max_single_length:])
